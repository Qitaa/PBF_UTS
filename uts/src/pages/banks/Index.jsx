...
export default function BankIndex() {
  ...
  const getBanks = async () => {
    let res = await axios.get('http://localhost:3001/bank')
    setBanks(res.data)
  }
  const handleModal = async (id) => {
    setModal(!modal)
    let data = await axios.get(`http://localhost:3001/bank/${id}`)
    setDataModal(data.data)
  }
  const handleDelete = async (id) => {
    try {
      const del = await axios.delete(`http://localhost:3001/bank/${id}`)
      if (del.status === 200) {
        getBanks()
        setModal(false)
      }
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <p className="text-2xl">Daftar Rekening</p>
