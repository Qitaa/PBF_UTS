...
export default function CategoryIndex() {
  ...
  const handleModal = async (id) => {
    setModal(!modal)
    let data = await axios.get(`http://localhost:3001/category/${id}`)
    setDataModal(data.data)
  }
  const handleDelete = async (id) => {
    try {
      const del = await axios.delete(`http://localhost:3001/category/${id}`)
      if (del.status === 200) {
        getCategory()
        setModal(false)
      }
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        ...
        <Link to={'/categoriesCreate'} className="bg-gray-200 btn btn-ghost"><i class="fa-solid fa-circle-plus mr-1"></i>Tambah</Link>
      </div>
