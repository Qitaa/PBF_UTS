...
export default function BrandIndex() {
  ...
  const getBrands = async () => {
    let res = await axios.get('http://localhost:3001/brand')
    setBrands(res.data)
  }
  const handleModal = async (id) => {
    setModal(!modal)
    let data = await axios.get(`http://localhost:3001/brand/${id}`)
    setDataModal(data.data)
  }
  const handleDelete = async (id) => {
    try {
      const del = await axios.delete(`http://localhost:3001/brand/${id}`)
      if (del.status === 200) {
        getBrands()
        setModal(false)
      }
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        ...
        <Link to={'/brandsCreate'} className="bg-gray-200 btn btn-ghost"><i class="fa-solid fa-circle-plus mr-1"></i>Tambah</Link>
      </div>
